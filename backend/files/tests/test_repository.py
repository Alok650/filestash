...
class GetApiKeyByTokenTests(TestCase):
    ...
    def test_finds_active_key(self):
        ...
        self.assertEqual(repository.get_api_key_by_token(raw), key)
...
class CreateFileTests(TestCase):
    ...
    def test_creates_record_with_correct_fields(self):
        key, _ = make_api_key()
        f = make_file(api_key=key, size=42, name='photo.png', mime='image/png',
                      sha256_hash='b' * 64)
    ...
    def test_rejects_invalid_sha256_hash(self):
        with self.assertRaises(ValueError):
            repository.create_file(
                file_field=make_uploaded_file(), original_filename='f.txt',
                file_type=VALID_MIME, size=5, sha256_hash='not-a-hash',
            )
    ...
    def test_rejects_uppercase_sha256_hash(self):
        with self.assertRaises(ValueError):
            repository.create_file(
                file_field=make_uploaded_file(), original_filename='f.txt',
                file_type=VALID_MIME, size=5, sha256_hash='A' * 64,
            )
    ...
    def test_rejects_short_sha256_hash(self):
        with self.assertRaises(ValueError):
            repository.create_file(
                file_field=make_uploaded_file(), original_filename='f.txt',
                file_type=VALID_MIME, size=5, sha256_hash='a' * 63,
            )
    ...
    def test_rejects_empty_original_filename(self):
        with self.assertRaises(ValueError):
            repository.create_file(
                file_field=make_uploaded_file(), original_filename='',
    ...
    def test_strips_whitespace_from_original_filename(self):
        f = repository.create_file(
            file_field=make_uploaded_file(), original_filename='  photo.png  ',
    ...
    def test_accepts_zero_size(self):
        f = repository.create_file(
            file_field=make_uploaded_file(content=b''), original_filename='empty.txt',
    ...
    def test_accepts_reused_path_string_for_dedup(self):
        ...
        f1 = make_file(sha256_hash='c' * 64)
        existing_path = f1.file.name
...
class GetFilesForKeyTests(TestCase):
    ...
    def test_returns_only_owned_files(self):
        key1, _ = make_api_key(label='k1')
        key2, _ = make_api_key(label='k2')
        f1 = make_file(api_key=key1)
        make_file(api_key=key2)
        self.assertQuerySetEqual(repository.get_files_for_key(key1), [f1], ordered=False)

    def test_returns_anonymous_files_for_none(self):
        key, _ = make_api_key()
        anon_file = make_file(api_key=None)
        make_file(api_key=key)
        self.assertQuerySetEqual(repository.get_files_for_key(None), [anon_file], ordered=False)
...
class GetAllFilesTests(TestCase):
    ...
    def test_returns_all_files_with_flag(self):
        key, _ = make_api_key()
        f1 = make_file(api_key=key)
        f2 = make_file(api_key=None)
...
class GetQuerysetForAuthTests(TestCase):
    ...
    def test_admin_sentinel_sees_all_files(self):
        ...
        self.assertQuerySetEqual(
            repository.get_queryset_for_auth(ADMIN_AUTH), [f1, f2], ordered=False
        )
    ...
    def test_api_key_sees_only_own_files(self):
        ...
        make_file(api_key=key2)
        self.assertQuerySetEqual(
            repository.get_queryset_for_auth(key1), [f1], ordered=False
        )

    def test_none_sees_only_anonymous_files(self):
        key, _ = make_api_key()
        make_file(api_key=key)
        anon = make_file(api_key=None)
        self.assertQuerySetEqual(
            repository.get_queryset_for_auth(None), [anon], ordered=False
        )

    def test_inactive_key_treated_as_anonymous(self):
        key, _ = make_api_key()
        keyed_file = make_file(api_key=key)
        anon_file = make_file(api_key=None)
        repository.deactivate_api_key(key)
        key.refresh_from_db()
...
class GetFileByHashTests(TestCase):
    ...
    def test_returns_first_when_multiple_share_hash(self):
        make_file(sha256_hash='f' * 64)
        make_file(sha256_hash='f' * 64)
        self.assertIsNotNone(repository.get_file_by_hash('f' * 64))
...
class GetDuplicatesForKeyTests(TestCase):
    ...
    def test_null_hash_returns_empty_queryset(self):
        key, _ = make_api_key()
        f1 = repository.create_file(
            file_field=make_uploaded_file(), original_filename='x.txt',
...
class CountReferencesTests(TestCase):
    ...
    def test_counts_all_keys(self):
        key1, _ = make_api_key(label='k1')
        key2, _ = make_api_key(label='k2')
        make_file(api_key=key1, sha256_hash='4' * 64)
        make_file(api_key=key2, sha256_hash='4' * 64)
...
class GetStorageUsedBytesTests(TestCase):
    ...
    def test_sums_sizes_for_key(self):
        key, _ = make_api_key()
        make_file(api_key=key, size=100)
        make_file(api_key=key, size=200)
        self.assertEqual(repository.get_storage_used_bytes(key), 300)

    def test_excludes_other_keys(self):
        key1, _ = make_api_key(label='k1')
        key2, _ = make_api_key(label='k2')
        make_file(api_key=key1, size=500)
        make_file(api_key=key2, size=999)
    ...
    def test_sums_anonymous_files_with_none(self):
        make_file(api_key=None, size=50)
        make_file(api_key=None, size=75)
        self.assertEqual(repository.get_storage_used_bytes(None), 125)

    def test_anonymous_excludes_keyed_files(self):
        key, _ = make_api_key()
        make_file(api_key=key, size=999)
        make_file(api_key=None, size=10)
...
class UpdateFileTests(TestCase):
    ...
    def test_no_op_when_no_args_given(self):
        f = make_file(name='unchanged.txt')
    ...
    def test_does_not_change_other_fields(self):
        key, _ = make_api_key()
        f = make_file(api_key=key, sha256_hash='6' * 64, size=42)
...
class DeleteFileTests(TestCase):
    ...
    def test_count_references_drives_correct_delete_choice(self):
        ...
        f1 = make_file(api_key=key1, sha256_hash=shared_hash)
        f2 = repository.create_file(
        ...
        # f1: another reference exists — keep disk file
        refs = repository.count_references(shared_hash, exclude_id=f1.pk)
        self.assertEqual(refs, 1)
        repository.delete_file(f1, delete_disk_file=(refs == 0))
        self.assertTrue(os.path.exists(path))

        # f2: last reference — remove disk file
        refs = repository.count_references(shared_hash, exclude_id=f2.pk)
        self.assertEqual(refs, 0)
        repository.delete_file(f2, delete_disk_file=(refs == 0))
        self.assertFalse(os.path.exists(path))
